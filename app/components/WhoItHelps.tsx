...
export default function WhoItHelps() {
  const segments = [
    ...
    { icon: "🏃", label: "Runners", headline: "Tight muscles are compensating for muscles that stopped firing.", detail: "Helpful for recurring hip, knee, IT band, and low-back issues when mobility and strength work haven't fully solved the pattern. MAT finds which muscles went offline.", href: "/conditions/chronic-pain" },
    ...
    { icon: "🏥", label: "Post-Surgery", headline: "We restore the signal to muscles your body shut down after surgery.", detail: "After injury or surgery, the nervous system inhibits muscles around the area. MAT identifies which ones lost their signal and gets them communicating again.", href: "/conditions/chronic-pain" },
    ...
    { icon: "🎯", label: "Told 'Everything Looks Fine'", headline: "Imaging shows structure. MAT looks at the signal.", detail: "A muscle can test clean on an MRI and still not communicate properly with the brain. That neurological gap is exactly what MAT addresses — and what most imaging misses.", href: "/conditions/chronic-pain" },
  ];
  return (
    <section id="who-it-helps" style={{ padding: "100px 48px", background: "var(--bg)", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 2, marginBottom: 2 }}>
          {segments.map((s, i) => (
            <a key={i} href={s.href} style={{ padding: "32px 28px", background: "var(--surface)", border: "1px solid var(--border)", textDecoration: "none", color: "inherit", display: "block", transition: "border-color 0.2s" }} onMouseOver={(e) => { e.currentTarget.style.borderColor = "var(--red)"; }} onMouseOut={(e) => { e.currentTarget.style.borderColor = "var(--border)"; }}>
        ...
        {/* Arnold Sports — stat callout + video side by side */}
