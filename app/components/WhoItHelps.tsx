export default function WhoItHelps() {
  const segments = [
    { icon: "⛳", label: "Golfers", headline: "Your swing isn't limited by flexibility. It's limited by muscles that have lost their signal.", detail: "For players restricted through the backswing, struggling to clear the hips, or wanting a body that can support a better swing. Rotation problems are often neuromuscular, not structural.", href: "/conditions/hip-pain" },
    ...
    { icon: "💼", label: "Desk Workers", headline: "Your neck pain keeps coming back because the signal is missing.", detail: "Built for the person whose neck, shoulders, or lower back tighten up no maow often they stretch. The compensating muscles keep getting loaded because the real muscles aren't firing.", href: null },
    { icon: "🏥", label: "Post-Surgery", headline: "We restore the signal to muscles your body shut down after surry.", detail: "After injury or surgery, the nervous system inhibits muscles around the area. MAT identifies which ones lost their signal and gets them communicating again.", href: "/conditions/post-surgical-recovery" },
    { icon: "🏋️", label: "Active Adults", headline: "Klifting, training, golfing without constantly managing flare-ups.", detail: "For people who want to stay active and strong without recurring tightness becoming the limiting factor. Neuromuscular gaps catch up with everyone eventually.", href: null },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 2, marginBottom: 2 }}>
          {segments.map((s, i) => (
            <div key={i} style={{ padding: "32px 28px", background: "var(--surface)", border: "1px solid var(--border)" }}>
              ...
              <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--muted)", lineHeight: 1.65, marginBottom: s.href ? 14 : 0 }}>{s.detail}</p>
              {s.href && (
                <a href={s.href} style={{ fontFamily: "var(--font-body)", fontSize: "0.72rem", color: "var(--red)", textDecoration: "none", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>Learn more →</a>
              )}
            </div>
          ))}
        </div>
